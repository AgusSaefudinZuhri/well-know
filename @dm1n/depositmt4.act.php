...
switch($_GET["a"]) {
	...
	if(count($json)>0) {
		for ($j=0; $j<count($json); $j++) {
			...
			$write1 = mysqli_query("UPDATE g_wmt4 SET status='1', tglapprove='".date('Y-m-d')."', waktuapprove='".date('H:i:s')."' WHERE id='".$id."'");
