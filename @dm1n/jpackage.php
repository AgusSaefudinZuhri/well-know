...
function tambah() {
	...
	if(cekzzz()) {	
		$('#dialog').dialog({
		title: '<?php echo T_("Add Package"); ?>',
		width: 600,
		height: 500,
		closed: false,
		cache: false,
		href: 'jpackage.add.php',
		modal: true  
		});
		loadingend ();
	}
...
function cari(x) {
	...
	if(cekzzz()) {	
		...
		var	fCari	= encodeURI( 'jpackage.f.php?pg='+x+'&pf='+pfilter+'&vf='+vfilter );		
...
function hapus (x1) {
	if(confirm( cDeActivate() )) {
		loadingstart();	
		if(cekzzz()) {		
			$.get('nonaktifk.php?t=5&id='+x1, function(e) {
				var hasil= e.trim();
				if(hasil=="success") {
					cari('');
...
function aktifk (x1) {
	if(confirm( cActivate() )) {
		loadingstart();	
		if(cekzzz()) {		
			$.get('aktifk.php?t=5&id='+x1, function(e) {
				var hasil= e.trim();
				if(hasil=="success") {
					cari('');
...
<a href="javascript:void(0);" class="tbutton tadd" onclick="tambah();" style="margin-top: 10px;"><?php echo T_("Package"); ?></a></td>
