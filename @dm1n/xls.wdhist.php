<?php
		...
		// HTTP headers
		HeaderingExcel(str_replace(" ", "_", "WITHDRAWAL HISTORY - ".date('d M Y His').".xls"));
...
if(isset($_GET["mf"]) and $_GET["mf"]!='') { $vf1 .= "Fr ".date('d M Y', strtotime($_GET["mf"])); $where.=" AND a.tglapprove >= '".$_GET["mf"]."' "; }
if(isset($_GET["nf"]) and $_GET["nf"]!='') { $vf1 .= " To ".date('d M Y', strtotime($_GET["nf"])); $where.=" AND a.tglapprove <= '".$_GET["nf"]."' "; }
...
//if($_GET["jf"]!='') $where.=" AND a.tgltransaksi = '".$_GET["jf"]."'"; 
		...
		$worksheet1->write_string(4,1, $param1);
		$worksheet1->write_string(4,3, $vf1);
		...
		$worksheet1->write_string(5,3,  "Proc. Date",$formatotA);
		$worksheet1->write_string(5,4,  "UserID",$formatotA);
		$worksheet1->write_string(5,5,  "Name",$formatotA);
		$worksheet1->write_string(5,6,  "Bank",$formatotA);
		$worksheet1->write_string(5,7,  "Branch",$formatotA);
		$worksheet1->write_string(5,8,  "Account No.",$formatotA);
		$worksheet1->write_string(5,9,  "Account Name",$formatotA);
		$worksheet1->write_string(5,10,  "Amount",$formatotA);
		$worksheet1->write_string(5,11,  "Fee",$formatotA);
		$worksheet1->write_string(5,12,  "Total",$formatotA);
		$worksheet1->write_string(5,13,  "Status",$formatotA);
		$worksheet1->write_string(5,14,  "Remark",$formatotA);
		...
		while($row=mysql_fetch_array($qry)) { 
			...
			$worksheet1->write_string($rowPos+$i,2,$tglinput,$formAll);
			$worksheet1->write_string($rowPos+$i,3,$tglapprove,$formAll);			
			$worksheet1->write_string($rowPos+$i,4,$row["userid"],$formAll);
			$worksheet1->write_string($rowPos+$i,5,$row["nmmember"],$formAll);
			$worksheet1->write_string($rowPos+$i,6,$row["nmbank"],$formAll);
			$worksheet1->write_string($rowPos+$i,7,$row["cabbank"],$formAll);
			$worksheet1->write_string($rowPos+$i,8,$row["norek"],$formAll);
			$worksheet1->write_string($rowPos+$i,9,$row["atasnama"],$formAll);
			$worksheet1->write_number($rowPos+$i,10,$row["withdrawal"],$formRight);
			$worksheet1->write_number($rowPos+$i,11,$row["admincharge"],$formRight);
			$worksheet1->write_number($rowPos+$i,12,$row["withdrawal"]+$row["admincharge"],$formRight);
			$worksheet1->write_string($rowPos+$i,13,$status,$formAll);
			$worksheet1->write_string($rowPos+$i,14,$remark2,$formAll);
