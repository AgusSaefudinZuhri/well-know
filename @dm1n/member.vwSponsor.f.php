...
$tqry="
	...
	FROM ".$table." a
	WHERE a.status >='0' AND jkomisi='1' AND jtransaksi='0' AND a.parentid='".$_GET["id"]."' ".$where;
...
<td style="text-align:left;"><?php echo $row["remark"];?> <?php switch($row["status"]) {
	...
	?></td>
<td style="text-align:right;"><?php if($row["jtransaksi"]=='0') echo number_format(($row["idrvalue"]*1),2); else echo '0.00'; ?></td>
<td style="text-align:right;"><?php if($row["jtransaksi"]=='1') echo number_format($row["idrvalue"],2); else echo '0.00'; ?></td>
</tr>
<?php $x++; } ?>
</table>
</div>
