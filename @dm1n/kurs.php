...
function tambah() {
	...
	if(cekzzz()) {	
		$('#dialog').dialog({
		title: '<?php echo T_("Add Currency"); ?>',
		width: 600,
		height: 500,
		closed: false,
		cache: false,
...
function edit(x) {
	...
	if(cekzzz()) {	
		$('#dialog').dialog({
		title: '<?php echo T_("Edit Currency");?>',
		width: 600,
		height: 500,
		closed: false,
		cache: false,
		href: 'kurs.e.php?id='+x,
		modal: true  
		});
		loadingend ();
	}
...
function cari(x) {
	...
	if(cekzzz()) {	
		...
		var	fCari	= encodeURI( 'kurs.f.php?pg='+x+'&pf='+pfilter+'&vf='+vfilter );		
...
<a href="javascript:void(0);" class="tbutton tadd" onclick="tambah();" style="margin-top: 10px;"><?php echo T_("Currency"); ?></a></td>
