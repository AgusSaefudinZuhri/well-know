...
function ddepositmt4Req (pfilter, vfilter) {
//	alert('xls.depositmt4req.php?pf='+pfilter+'&vf='+vfilter);
//		window.open('xls.depositmt4req.php?pf='+pfilter+'&vf='+vfilter, '_blank');
}
...
$.messager.confirm('<?php echo T_(" Confirmation"); ?>','<?php echo T_("Confirm to Reject?"); ?>',function(r0){
	if (r0){
	$.get('depositmt4.act.php?a=2&x='+x, function(e) {
		var hasil = $.trim(e);
        if(hasil=="success") {
			cari('');
...
$.messager.confirm('<?php echo T_("Payment Confirmation"); ?>','<?php echo T_("Confirm to Pay?"); ?>',function(r0){
	if (r0){
		...
		$.get('depositmt4req.act.php?a=1&m='+selected, function(e) {
			var hasil = $.trim(e);
			if(hasil=="success") {
				cari('');
				$.messager.alert( sucProc(), "<?php echo T_("Transcation is successful!!!"); ?>","info");
...
//		var userid = $('#userid').val();
		...
		var selected =  JSON.stringify(warray); 
	//	alert(selected);
		$.get('depositmt4req.act.php?a=3&m='+selected, function(e) {
			var hasil = $.trim(e);
			if(hasil=="success") {
				cari('');
				$.messager.alert( sucProc(), "<?php echo T_("Transcation is successful!!!"); ?>","info");
...
function cari(x) {
	...
	if(cekzzz()) {	
		...
		var	fCari	= encodeURI( 'depositmt4req.f.php?pg='+x+'&pf='+pfilter+'&vf='+vfilter);	
...
function tDownload() {
	var pfilter		= $("#pfilter").val();	
	var vfilter		= $("#vfilter").val();
	var	fDownload	= encodeURI('xls.depositmt4req.php?pf='+pfilter+'&vf='+vfilter);	
...
<div id="iwrapper">
<?php include('depositmt4req.f.php'); ?>
<br/>
</div>
</div>