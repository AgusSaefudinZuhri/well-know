...
function tambah() {
	...
	if(cekzzz()) {	
		$('#dialog').dialog({
		title: '<?php echo T_("Add Message"); ?>',
		width: 600,
		height: 500,
		closed: false,
		cache: false,
		href: 'support.add.php',
		modal: true  
		});
		loadingend ();
	}
...
function edit(x) {
	...
	if(cekzzz()) {	
		$('#dialog').dialog({
		title: '<?php echo T_("Reply Message");?>',
		width: 600,
		height: 500,
		closed: false,
		cache: false,
		href: 'support.e.php?id='+x,
		modal: true  
		});
		loadingend ();
	}
...
function cari(x) {
	...
	if(cekzzz()) {	
		...
		var	fCari	= encodeURI( 'support.f.php?pg='+x+'&pf='+pfilter+'&vf='+vfilter );		
...
<!--<a href="javascript:void(0);" class="tbutton tadd" onclick="tambah();" style="margin-top: 10px;"><?php echo T_("Message"); ?></a>--></td>
<td style="width: 50%; text-align: right;">
<select id="pfilter" class="default" >
    <option value="1"><?php echo T_("UserID"); ?></option>
</select>&nbsp;&nbsp;&nbsp;
