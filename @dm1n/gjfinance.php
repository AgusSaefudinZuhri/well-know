...
function tambah() {
	...
	if(cekzzz()) {	
		$('#dialog').dialog({
		title: '<?php echo T_("Add Finance Group"); ?>',
		width: 600,
		height: 500,
		closed: false,
		cache: false,
		href: 'gjfinance.add.php',
		modal: true  
		});
		loadingend ();
	}
...
function edit(x) {
	...
	if(cekzzz()) {	
		$('#dialog').dialog({
		title: '<?php echo T_("Edit Finance Group"); ?>',
		width: 600,
		height: 500,
		closed: false,
		cache: false,
		href: 'gjfinance.e.php?id='+x,
		modal: true  
		});
		loadingend ();
	}
...
function cari(x) {
	...
	if(cekzzz()) {	
		...
		$('#iwrapper').load('gjfinance.f.php?pg='+x+'&pf='+pfilter+'&vf='+vfilter, function () {loadingend();});
	}
	else  clogout ();
}
...
function hapus (x1) {
	if(confirm( cDeActivate() )) {
		loadingstart();	
		if(cekzzz()) {		
			$.get('nonaktifk.php?t=16&id='+x1, function(e) {
				var hasil= e.trim();
				if(hasil=="success") {
					cari('');
...
function aktifk (x1) {
	if(confirm( cActivate() )) {
		loadingstart();	
		if(cekzzz()) {		
			$.get('aktifk.php?t=16&id='+x1, function(e) {
				var hasil= e.trim();
				if(hasil=="success") {
					cari('');
...
<a href="javascript:void(0);" class="tbutton tadd" onclick="tambah();" style="margin-top: 10px;"><?php echo T_("Add Finance Group");?></a></td>
<td style="width: 50%; text-align: right;">
<select id="pfilter" class="default" >
    <option value="1" selected="selected"><?php echo T_("Finance Group");?></option>
</select>&nbsp;&nbsp;&nbsp;
